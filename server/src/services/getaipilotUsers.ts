...
export async function fetchGetAiPilotUsers() {
  ...
  const mappedProfiles = profiles.map((profile, idx) => {
    const email = profile.email || profile.phone || `user_${idx}@getaipilot.in`;
    ...
    return {
      ...
      status: (authUser as any)?.banned_until ? 'blocked' : 'active',
  ...
  const authOnlyUsers = authUsers
    .filter((user: any) => {
      ...
      return user.email && !profileIds.has(user.id) && !profileEmails.has(email);
    })
    .map((user: any, idx) => ({
      id: user.id,
      email: user.email,
